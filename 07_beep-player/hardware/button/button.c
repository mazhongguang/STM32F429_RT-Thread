...
#define MY_BUTTON_CALL(func,argv) do{if((func) != RT_NULL) func argv;}while(0)

struct my_button_manage
{
	rt_uint8_t num;		/* 需要注册的按键数 */
	rt_timer_t timer;	/* soft timer */
...
static void my_button_scan(void *param)
{
	...
	for (i = 0; i < button_manage.num; i++)
	{
		...
		if (rt_pin_read(button_manage.button_list[i]->pin) == button_manage.button_list[i]->press_logic_level)
		{
			...
			if (button_manage.button_list[i]->cnt == MY_BUTTON_DOWN_MS / MY_BUTTON_SCAN_SPACE_MS)		//button down
			{
				LOG_D("BUTTON_DOWN");
			...
			else if (button_manage.button_list[i]->cnt == MY_BUTTON_HOLD_MS / MY_BUTTON_SCAN_SPACE_MS)	//button hold
			{
				LOG_D("BUTTON_HOLD");
...
int my_button_start(void)
{
	...
	/*创建定时器1*/
	button_manage.timer = rt_timer_create("timer1",				/* tiemr's name -> timer1 */
										my_button_scan,			/* timeout callback function */
										RT_NULL,				/* timeout function's entry param */
										RT_TICK_PER_SECOND * MY_BUTTON_SCAN_SPACE_MS / 1000,	/* timer's time, unit->OS Tick,(10 OS Tick) */
										RT_TIMER_FLAG_PERIODIC | RT_TIMER_FLAG_SOFT_TIMER);		/* periodic timer */
